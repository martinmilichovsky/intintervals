...
class TimeImplicitsSpec extends FunSuite {
  ...
  test("time implicit") {
    ...
    val weekendNightHoursWorked = workingIntervals.filter(Query.key("SATURDAY") and "night" or (Query.key("SUNDAY") and "night")).duration
    val nightHoursWorked = workingIntervals.filter(Query.empty and "night" andNot "SATURDAY" andNot "SUNDAY").duration
    val weekendHoursWorked = workingIntervals.filter(Query.key("SATURDAY") andNot "night" or (Query.key("SUNDAY") andNot "night")).duration
    ...
    for (offset <- 0 to 1000) {
...

      {
        ...
        assert(t0.getDayOfYear - 1 ===
          ints.filter(Query.key("last 365 days") and "this year").duration)
      }
      ...
      assert((weeksInYear to weeksInYear + 1) contains
        ints.filter(Query.key("sundays +-2yrs") and "this year" and "last 365 days").duration.getDays,
